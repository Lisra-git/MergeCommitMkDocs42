...
On peut imaginer qu'un programme est une suite finie de choix, permettant de s'adapter aux situations rencontrées. Ce type de choix s'appelle un branchement conditionnel. CONDITONNEL / INCONDITIONNEL ??? Voir livre agreg.

!!! tip inline end "_Flowchart_"
    ```mermaid
    flowchart TD
        A-->p[<Condition>]
        p-->|Vrai|B
...
!!! {{cours()}}
    Une conditionnelle permet d'effectuer une certaine action si une certaine condition est remplie.

!!! tip "Comprendre les conditionnelles"

    === {{exercice(False, 0)}}
        Voici un programme écrit en Python. Dessinez le _flowchart_ correspondant.
        ```python
        age_client = 15
        if age_client <= 12 :
            prix = 5
...
!!! {{cours()}}
    ...
    Pour poser une question, on utilise des symboles mathémtiques : `<=`, `<` `==`
...
!!! tip "Choisissez la syntaxe correcte"
    ...
    === "Question"
        ...
        - [ ] Proposition 4
            ```python 
            if nombre_points == 20 : niveau = niveau + 1
            ```
    ...
    === "Réponse"
        ...
        - [ ] Proposition 3 : attention aux deux-points manquants.
            ...
            if nombre_points == 20
                niveau = niveau + 1
            ```
        - [ ] Proposition 4 : correcte mais ne fonctionne qu'avec une seule instruction.
            ```python 
            if nombre_points == 20 : niveau = niveau + 1
            ```             
